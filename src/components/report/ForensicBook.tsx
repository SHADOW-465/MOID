...
// Full 24-page forensic quality book — rendered as one report preset.
// Extracted from the old standalone /reports page so it can live inside the
// Reports editor as a named preset rather than a separate product.
...
/** Print geometry for the forensic book (A4 pages). */
export const FORENSIC_PRINT_CSS = `
@media screen {
  .forensic-book .print-report-container { max-width: 820px; margin: 0 auto; }
  .forensic-book .pdf-page-wrapper {
    background: #FFFFFF; color: #14181f; border: 1px solid var(--border);
    box-shadow: 0 4px 16px rgba(0, 0, 0, 0.08); margin-bottom: 32px;
    height: 297mm; aspect-ratio: 210/297; width: 100%; padding: 20mm;
    display: flex; flex-direction: column; justify-content: space-between;
    box-sizing: border-box; position: relative;
  }
  .forensic-book .page-content-wrapper {
...
export default function ForensicBook({
  ...
  if (!m) {
    return (
      <div className="py-12 text-center text-muted border border-dashed border-hairline rounded-lg">
        No data yet. Import an Excel file, or log a shift on Data Entry, and the forensic book builds itself.
      </div>
    );
  }

  return (
    <div className="w-full pb-12 forensic-book">
      <style dangerouslySetInnerHTML={{ __html: FORENSIC_PRINT_CSS }} />
      {(() => {
